...
namespace word2vector_word2vector {
...
#ifdef __DEBUG_CONFIGURATION__
  cerr<<"Configuration::CheckParamsInConfig...\n"<<flush;
#endif
...
#ifdef __DEBUG_CONFIGURATION__
  cerr<<"Configuration::CheckFilesInConfig...\n"<<flush;
#endif
...
bool Word2Vector::TrainModel(map<string, string> &parameters) { 
  cerr<<"Train Word2Vector Model...\n"<<flush;
  config_.Init(parameters);

#ifdef __DEBUG_WORD2VECTOR__
  int test_int_a = 5;
  char test_char_a = test_int_a;
  cerr<<"test_int_a="<<test_int_a<<"\n"<<flush;
  cerr<<"test_char_a="<<test_char_a<<"\n"<<flush;
  cerr<<"1-test_int_a="<<1-test_int_a<<"\n"<<flush;
  cerr<<"1-test_char_a="<<1-test_char_a<<"\n"<<flush;
#endif
...
#ifdef __DEBUG_WORD2VECTOR__
  cerr<<"id_="<<id<<" "<<"train_file_size="<<config_.train_file_size_<<" seek_position="<<seek_position<<"\n"<<flush;
#endif
...
#ifdef __DEBUG_WORD2VECTOR__
  cerr<<"exp_table.capacity="<<exp_table_.capacity()<<"\n"<<flush;
#endif
...
#ifdef __DEBUG_WORD2VECTOR__
  cerr<<"exp_table.capacity="<<exp_table_.capacity()<<"\n"<<flush;
  cerr<<"config_.exp_table_size_="<<config_.exp_table_size_<<"\n"<<flush;
  cerr<<"config_.max_exp_="<<config_.max_exp_<<"\n"<<flush;
#endif
...
#ifdef __DEBUG_WORD2VECTOR__
  cerr<<"vocab_hash.capacity="<<vocab_hash_.capacity()<<"\n"<<flush;
#endif

//  vocab_hash_.reserve(config_.vocab_hash_size_);
...
bool Word2Vector::OutputModel() {
  cerr<<"output_file="<<config_.output_file_<<"\n"<<flush;
